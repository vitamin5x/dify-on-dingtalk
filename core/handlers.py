...
# 在 DifyAiCardBotHandler 类中添加以下修改
class DifyAiCardBotHandler(ChatbotHandler):
    def __init__(self, client_class, api_key, base_url, pool_size=5):
        super().__init__()
        self.client_pool = [client_class(api_key, base_url) for _ in range(pool_size)]
        self.pool_lock = Lock()
    ...
    async def process(self, callback_msg: CallbackMessage):
        self.message_queue.put(callback_msg)
        self._process_queue()
        return AckMessage.STATUS_OK, "OK"

    def _process_queue(self):
        if not self.processing_lock.locked() and not self.message_queue.empty():
            with self.processing_lock:
                while not self.message_queue.empty():
                    callback = self.message_queue.get()
                    self._real_process(callback)

    def _real_process(self, callback_msg: CallbackMessage):
        thread_id = threading.get_ident()
        # 原错误代码：使用 callback_msg.message_id ↓↓↓
        logger.debug(f"[Thread-{thread_id}] 开始处理消息 {callback_msg.data.get('messageId')}")
        ...
        try:
            ...
            # 同步修改完成日志 ↓↓↓
            logger.debug(f"[Thread-{thread_id}] 完成处理 {callback_msg.data.get('messageId')}")
